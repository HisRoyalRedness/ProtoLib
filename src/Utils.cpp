...
uint32_t BinToHexString(const uint8_t* in_buf, uint32_t in_len, char* out_buf, uint32_t out_len)
{
    ...
    // Work out how many bytes we can convert
    // We're converting 1 byte to 3 ascii chars, e.g 0x12 => "12 "
    // Note the trailing space
    uint32_t max_bytes = out_len / 3;
    if (in_len < max_bytes)
        max_bytes = in_len;

    for (uint8_t i = 0; i < max_bytes; ++i)
    {
        *(p_out++) = hex[((static_cast<char>(in_buf[i])) >> 4) & 0xF];
        *(p_out++) = hex[(static_cast<char>(in_buf[i])) & 0xF];
        *(p_out++) = (i + 1 == max_bytes ? '\0' : ' ');
    }
